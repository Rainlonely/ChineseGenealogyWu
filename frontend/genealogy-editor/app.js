...
function renderWorkspaceSummary(summary) {
  ...
  workspaceSummaryText.textContent =
    summary.range_ready
      ? "1-102世已连成完整树，数据库状态正常。"
      : `1-102世仍有 ${summary.range_missing_parent_count || 0} 个未接上的父链。`;
  const stats = [
    {
      label: "树状态",
...
function nextMergeActionContext(data = state.data) {
  ...
  if (lastIndex < 0 || lastIndex >= GROUPS.length - 1) return null;
...
function shouldKeepFullscreenCrossPageContext() {
  return document.fullscreenElement === graphWrap && state.linkMode;
}
...
function renderFullscreenPageImages({ pages, pageWidthMap, width, pageGap, rightMargin }) {
  ...
  const orderedPages = mergeMode
    ? pages.filter((page) => ["bottom", "single"].includes(mergeRowKeyForPage(page))).slice().reverse()
    : pages.slice().reverse();
    ...
    .map((page) => {
      ...
      return `
        <figure class="fullscreen-page-image-item${Number(page) === Number(state.page) ? " active" : ""}" data-fullscreen-page="${page}" style="width:${itemWidth}px">
          <figcaption class="fullscreen-page-image-caption">${mergeMode ? `第${page}页原稿` : `第${page}页`}</figcaption>
          <img src="${escapeHtml(entry?.image || "")}" alt="第${page}页完整原图" />
        </figure>
      `;
    })
    .join("");
...
function renderPageGroupedFullscreenGraph(graphNodes, graphEdges) {
  ...
  pages.forEach((page) => {
    ...
    pageWidthMap.set(page, Math.max(220, pagePadding * 2 + Math.max(cursorX - leafUnit + nodeWidth, nodeWidth)));
  });
...
async function saveData(isAuto = false) {
  ...
  const dbSummary = payload.sqlite_mirror?.skipped
    ...
    : payload.sqlite_mirror?.error
      ...
      : "";
  updateOverlayHint();
  setStatus((isAuto ? "已自动保存" : saveTarget ? `已保存到 ${saveTarget}` : "已保存") + dbSummary);
...
function applyLoadedData(data, preferredPage = null) {
  ...
  pageSelect.innerHTML = state.data.pages
    .map((page) => `<option value="${page}">第${page}页</option>`)
    .join("");
  ensureActivePersonForPage();
  switchPage(state.page);
  refreshUndoAvailability();
